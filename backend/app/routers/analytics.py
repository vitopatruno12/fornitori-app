...
def analytics_snapshot(
    ...
    try:
        return analytics_service.get_snapshot(model_id=mid, months=months)
...
def analytics_daily(
    ...
    mid = model_id or activity
    try:
        return analytics_service.get_daily_series(days=days, model_id=mid)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Analisi VNE non disponibile: {e}") from e
...
def analytics_weekly(
    ...
    mid = model_id or activity
    try:
        return analytics_service.get_weekly_series(weeks=weeks, model_id=mid)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Analisi VNE non disponibile: {e}") from e
...
def analytics_monthly(
    ...
    mid = model_id or activity
    try:
        return analytics_service.get_monthly_series(months=months, model_id=mid)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Analisi VNE non disponibile: {e}") from e
