...
export default function OperatorStationApp() {
  ...
  const active = STATION_SECTIONS.find((s) => s.id === view) || STATION_SECTIONS[1]

  return (
    <OperatorSatelliteShell
      documentTitle={`ATLAS — ${active.title} (postazione)`}
      loginHint="Accesso postazione operativa — solo Personale, Nuovo ordine e Prima Nota"
      headerTitle="Postazione operativa"
      headerSubtitle={`${active.title} — accesso limitato alle tre sezioni operative`}
      stationOnly
